...
{
    if ([elementName compare:@"sensorItem" options:NSCaseInsensitiveSearch]) {
        self.sensorItem = [[SensorItem alloc] init];
    }
    if ([elementName compare:@"timestamp" options:NSCaseInsensitiveSearch]) {
        self.timestamp = [[Timestamp alloc] init];
    }
    if ([elementName compare:@"value" options:NSCaseInsensitiveSearch]) {
...
{
    if ([elementName compare:@"sensorId" options:NSCaseInsensitiveSearch]) {
        _sensorItem.sensorId = [NSString stringWithString:_tagContent];
    }
    if ([elementName compare:@"type" options:NSCaseInsensitiveSearch]) {
        _sensorItem.type = [NSString stringWithString:_tagContent];
    }
    if ([elementName compare:@"value" options:NSCaseInsensitiveSearch]) {
        _sensorValue.value = [NSString stringWithString:_tagContent];
    }
    if ([elementName compare:@"unit" options:NSCaseInsensitiveSearch]) {
        _sensorValue.unit = [NSString stringWithString:_tagContent];
    }
    if ([elementName compare:@"subType" options:NSCaseInsensitiveSearch]) {
        _sensorValue.subType = [NSString stringWithString:_tagContent];
    }
    if ([elementName compare:@"hour" options:NSCaseInsensitiveSearch]) {
        _timestamp.hour = [_tagContent integerValue];
    }
    if ([elementName compare:@"minute" options:NSCaseInsensitiveSearch]) {
        _timestamp.minute = [_tagContent integerValue];
    }
    if ([elementName compare:@"second" options:NSCaseInsensitiveSearch]) {
        _timestamp.second = [_tagContent integerValue];
    }
    if ([elementName compare:@"year" options:NSCaseInsensitiveSearch]) {
        _timestamp.year = [_tagContent integerValue];
    }
    if ([elementName compare:@"month" options:NSCaseInsensitiveSearch]) {
        _timestamp.month = [_tagContent integerValue];
    }
    if ([elementName compare:@"day" options:NSCaseInsensitiveSearch]) {
        _timestamp.day = [_tagContent integerValue];
    }
    if ([elementName compare:@"values" options:NSCaseInsensitiveSearch]) {
