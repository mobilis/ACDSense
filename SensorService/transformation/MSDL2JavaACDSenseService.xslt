...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<!-- the output folder where the classes will be created. On Windows this folder must exist. -->
	<xsl:param name="outputFolder" select="'build/service/'"/>
	
	<!-- set package namespace if required. default = '' -->
	<xsl:param name="packageNamespace" select="'service.package.aservice.proxy'" />
    ...
    <xsl:template match="xs:element" mode="parseElementDatatype" >
    	...
    	<xsl:choose>
			...
			<xsl:when test="compare(./@maxOccurs, 'unbounded') = 0 and number($asSimple) != 1">
				...
				<xsl:choose>
					<xsl:when test="contains($elementType,'mns:')">
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="contains($elementType,'mns:') or compare($elementType,'xs:int') = 0 or compare($elementType,'xs:long') = 0 or compare($elementType,'xs:boolean') = 0 or compare($elementType,'xs:double') = 0 or compare($elementType,'xs:float') = 0 or compare($elementType,'xs:byte') = 0 or compare($elementType,'xs:short') = 0">
						<xsl:value-of select="substring-after($elementType,':')" />
					</xsl:when>
					...
					<xsl:when test="compare(./@maxOccurs, 'unbounded') = 0">
						<xsl:choose>
							...
							<xsl:when test="starts-with(./@type,'mns:')">
								...
								<xsl:value-of select="substring-after(@type,'mns:')" />
									<xsl:text> entity = new </xsl:text><xsl:value-of select="substring-after(@type,'mns:')" /><xsl:text>();</xsl:text>
